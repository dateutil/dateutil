...
from dateutil import tz as tz, zoneinfo
from dateutil.parser import parse
# dateutil imports
from dateutil.relativedelta import SU, TH, relativedelta

from ._common import ComparesEqual, PicklableMixin, TZEnvContext, TZWinContext, WarningTestMixin
...
MISSING_TARBALL = (
    "This test fails if you don't have the dateutil "
...
class TzFoldMixin(object):
    ...
    def testAmbiguousNegativeUTCOffset(self):
        self._test_ambiguous_time(
            datetime(2015, 11, 1, 1, 30), "America/New_York", True
        )
    ...
    def testUnambiguousNegativeUTCOffset(self):
        self._test_ambiguous_time(
            datetime(2015, 11, 1, 2, 30), "America/New_York", False
        )

    def testUnambiguousPositiveUTCOffset(self):
        self._test_ambiguous_time(
            datetime(2012, 4, 1, 3, 30), "Australia/Sydney", False
        )

    def testUnambiguousGapNegativeUTCOffset(self):
        # Imaginary time
        self._test_ambiguous_time(
            datetime(2011, 3, 13, 2, 30), "America/New_York", False
        )

    def testUnambiguousGapPositiveUTCOffset(self):
        # Imaginary time
        self._test_ambiguous_time(
            datetime(2012, 10, 7, 2, 30), "Australia/Sydney", False
        )
    ...
    def testImaginaryNegativeUTCOffset(self):
        self._test_imaginary_time(
            datetime(2011, 3, 13, 2, 30), "America/New_York", False
        )

    def testNotImaginaryNegativeUTCOffset(self):
        self._test_imaginary_time(
            datetime(2011, 3, 13, 1, 30), "America/New_York", True
        )
    ...
    def testNotImaginaryPositiveUTCOffset(self):
        self._test_imaginary_time(
            datetime(2012, 10, 7, 1, 30), "Australia/Sydney", True
        )
...
class TzOffsetTest(unittest.TestCase):
    ...
    def testTimeOnlyOffset(self):
        ...
        self.assertEqual(
            dt_time(13, 20, tzinfo=tz_offset).utcoffset(), timedelta(seconds=3600)
        )
...
@unittest.skipUnless(
    TZEnvContext.tz_change_allowed(), TZEnvContext.tz_change_disallowed_message()
)
class TzLocalNixTest(unittest.TestCase, TzFoldMixin):
    ...
    def testTimeOnlyOffsetLocalUTC(self):
        with TZEnvContext(self.UTC):
            self.assertEqual(
                dt_time(13, 20, tzinfo=tz.tzlocal()).utcoffset(), timedelta(0)
            )

    def testTimeOnlyOffsetLocalDST(self):
        with TZEnvContext(self.TZ_EST):
            self.assertIs(dt_time(13, 20, tzinfo=tz.tzlocal()).utcoffset(), None)

    def testTimeOnlyDSTLocalUTC(self):
        with TZEnvContext(self.UTC):
            self.assertEqual(dt_time(13, 20, tzinfo=tz.tzlocal()).dst(), timedelta(0))

    def testTimeOnlyDSTLocalDST(self):
        with TZEnvContext(self.TZ_EST):
            self.assertIs(dt_time(13, 20, tzinfo=tz.tzlocal()).dst(), None)
...
@mark_tzlocal_nix
@pytest.mark.parametrize(
    "tzvar", ["Europe/London", "America/New_York", "GMT0BST", "EST5EDT"]
)
...
class ZoneInfoGettzTest(GettzTest, WarningTestMixin):
    ...
    def testZoneInfoFileStart1(self):
        ...
        self.assertEqual(
            datetime(2003, 4, 6, 1, 59, tzinfo=tz).tzname(), "EST", MISSING_TARBALL
        )
    ...
    def testZoneInfoFileEnd1(self):
        ...
        self.assertEqual(
            datetime(2003, 10, 26, 0, 59, tzinfo=tzc).tzname(), "EDT", MISSING_TARBALL
        )
...
class TZRangeTest(unittest.TestCase, TzFoldMixin):
    ...
    def testTimeOnlyRangeFixed(self):
        ...
        self.assertEqual(
            dt_time(13, 20, tzinfo=tz_range).utcoffset(), timedelta(hours=-3)
        )
    ...
    def testBrokenIsDstHandling(self):
        ...
        self.assertEqual(
            dt.astimezone(tz=tz.gettz("GMT+2")),
    ...
    def testRangeEquality(self):
        ...
        TZR6 = tz.tzrange(
            ...
            "EDT",
            -14400,
            start=relativedelta(hours=+1, month=3, day=1, weekday=SU(+2)),
        )
        ...
        TZR7 = tz.tzrange(
            ...
            "EDT",
            -14400,
            end=relativedelta(hours=+1, month=11, day=1, weekday=SU(+2)),
        )
...
class TZStrTest(unittest.TestCase, TzFoldMixin):
    ...
    def testStrStr(self):
        ...
        self.assertEqual(
            datetime(2003, 4, 6, 1, 59, tzinfo=tz.tzstr(str("EST5EDT"))).tzname(), "EST"
        )
        self.assertEqual(
            datetime(2003, 4, 6, 2, 00, tzinfo=tz.tzstr(str("EST5EDT"))).tzname(), "EDT"
        )
    ...
    def testPosixOffset(self):
        TZ1 = tz.tzstr("UTC-3")
        self.assertEqual(
            datetime(2015, 1, 1, tzinfo=TZ1).utcoffset(), timedelta(hours=-3)
        )

        TZ2 = tz.tzstr("UTC-3", posix_offset=True)
        self.assertEqual(
            datetime(2015, 1, 1, tzinfo=TZ2).utcoffset(), timedelta(hours=+3)
        )
...
@pytest.mark.tzstr
@pytest.mark.parametrize(
    "tzstr_1", ["EST5EDT", "EST5EDT4,M4.1.0/02:00:00,M10-5-0/02:00"]
)
@pytest.mark.parametrize(
    "tzstr_2", ["EST5EDT", "EST5EDT4,M4.1.0/02:00:00,M10-5-0/02:00"]
)
...
class TZICalTest(unittest.TestCase, TzFoldMixin):
    ...
    def _test_us_zone(self, tzc, func, values, start):
        ...
        dts = (
            tz.enfold(dt.replace(tzinfo=tzc), fold=f) for dt, f in zip((dt1, dt2), fold)
        )
    ...
    def _testMultizone(self, start, test_type):
        tzstrs = (
            self._gettz_str("America/New_York"),
...
class TZTest(unittest.TestCase):
    ...
    def testRoundNonFullMinutes(self):
        ...
        self.assertEqual(
            str(datetime(1900, 1, 1, 0, 0, tzinfo=tzc)), "1900-01-01 00:00:00+01:40"
        )
    ...
    def testGMTOffset(self):
        ...
        dt = datetime(2007, 8, 6, 4, 10, tzinfo=tz.tzutc())
        self.assertEqual(
            dt.astimezone(tz=tz.tzstr("GMT+2")),
            datetime(2007, 8, 6, 6, 10, tzinfo=tz.tzstr("GMT+2")),
        )
        self.assertEqual(
            dt.astimezone(tz=tz.gettz("UTC-2")),
...
class TzWinTest(unittest.TestCase, TzWinFoldMixin):
    ...
    def testTzResNameFromString(self):
        tzr = tzwin.tzres()
        self.assertEqual(
            tzr.name_from_string("@tzres.dll,-221"), "Alaskan Daylight Time"
        )

        self.assertEqual(
            tzr.name_from_string("Samoa Daylight Time"), "Samoa Daylight Time"
        )
    ...
    def testOffset(self):
        ...
        self.assertEqual(
            tz.utcoffset(datetime(1995, 5, 21, 12, 9, 13)), timedelta(-1, 82800)
        )
    ...
    def testTzwinTimeOnlyDST(self):
        ...
        self.assertEqual(dt_time(14, 10, tzinfo=tw_sast).dst(), timedelta(0))
    ...
    def testTzwinTimeOnlyUTCOffset(self):
        ...
        tw_sast = tz.tzwin("South Africa Standard Time")
        self.assertEqual(
            dt_time(14, 10, tzinfo=tw_sast).utcoffset(), timedelta(hours=2)
        )
    ...
    def testTzwinTimeOnlyTZName(self):
        ...
        tw_est = tz.tzwin("Eastern Standard Time")
        self.assertEqual(
            dt_time(14, 10, tzinfo=tw_est).tzname(), "Eastern Standard Time"
        )

        # For zones with no DST, this should work normally.
        tw_sast = tz.tzwin("South Africa Standard Time")
        self.assertEqual(
            dt_time(14, 10, tzinfo=tw_sast).tzname(), "South Africa Standard Time"
        )


@unittest.skipUnless(IS_WIN, "Requires Windows")
@unittest.skipUnless(
    TZWinContext.tz_change_allowed(), TZWinContext.tz_change_disallowed_message()
)
class TzWinLocalTest(unittest.TestCase, TzWinFoldMixin):
    ...
    def testTzwinLocalUTCOffset(self):
        with TZWinContext("Eastern Standard Time"):
            ...
            self.assertEqual(
                datetime(2014, 3, 11, tzinfo=tzwl).utcoffset(), timedelta(hours=-4)
            )
    ...
    def testTzwinLocalTimeOnlyTZName(self):
        ...
        with TZWinContext("South Africa Standard Time"):
            twl = tz.tzwinlocal()
            self.assertEqual(
                dt_time(14, 10, tzinfo=twl).tzname(), "South Africa Standard Time"
            )
...
class DatetimeAmbiguousTest(unittest.TestCase):
    ...
    def testNoSupportAmbiguityUnambiguousAware(self):
        ...
        self.assertFalse(
            tz.datetime_ambiguous(datetime(2018, 10, 1, 12, 30, tzinfo=tzi))
        )
