...
from ._common import NotAValue, WarningTestMixin
...
class RelativeDeltaTest(WarningTestMixin, unittest.TestCase):
    ...
    def testInheritance(self):
        ...
        self.assertEqual(
            type(ccRD + rd), type(ccRD), msg="Addition does not inherit type."
        )

        self.assertEqual(
            type(ccRD - rd), type(ccRD), msg="Subtraction does not inherit type."
        )

        self.assertEqual(type(-ccRD), type(ccRD), msg="Negation does not inherit type.")

        self.assertEqual(
            type(ccRD * 5.0), type(ccRD), msg="Multiplication does not inherit type."
        )
    ...
    def testNextFridayInt(self):
        self.assertEqual(
            self.today + relativedelta(weekday=calendar.FRIDAY), date(2003, 9, 19)
        )

    def testLastFridayInThisMonth(self):
        self.assertEqual(
            self.today + relativedelta(day=31, weekday=FR(-1)), date(2003, 9, 26)
        )
    ...
    def testNextWenesdayNotToday(self):
        self.assertEqual(
            self.today + relativedelta(days=+1, weekday=WE), date(2003, 9, 24)
        )
    ...
    def testYearDay(self):
        self.assertEqual(
            date(2003, 1, 1) + relativedelta(yearday=260), date(2003, 9, 17)
        )
        self.assertEqual(
            date(2002, 1, 1) + relativedelta(yearday=260), date(2002, 9, 17)
        )
        self.assertEqual(
            date(2000, 1, 1) + relativedelta(yearday=260), date(2000, 9, 16)
        )
    ...
    def testYearDayBug(self):
        ...
        self.assertEqual(
            date(2010, 1, 1) + relativedelta(yearday=15), date(2010, 1, 15)
        )
    ...
    def testAddition(self):
        self.assertEqual(
            ...
            relativedelta(
                years=1, months=2, days=13, hours=4, minutes=5, microseconds=6
            ),
        )

    def testAbsoluteAddition(self):
        self.assertEqual(
            relativedelta() + relativedelta(day=0, hour=0), relativedelta(day=0, hour=0)
        )
        self.assertEqual(
            relativedelta(day=0, hour=0) + relativedelta(), relativedelta(day=0, hour=0)
        )
    ...
    def testAdditionFloatValue(self):
        self.assertEqual(
            datetime(2000, 1, 1) + relativedelta(days=float(1)), datetime(2000, 1, 2)
        )
        self.assertEqual(
            datetime(2000, 1, 1) + relativedelta(months=float(1)), datetime(2000, 2, 1)
        )
        self.assertEqual(
            datetime(2000, 1, 1) + relativedelta(years=float(1)), datetime(2001, 1, 1)
        )

    def testAdditionFloatFractionals(self):
        self.assertEqual(
            datetime(2000, 1, 1, 0) + relativedelta(days=float(0.5)),
            datetime(2000, 1, 1, 12),
        )
        self.assertEqual(
            datetime(2000, 1, 1, 0, 0) + relativedelta(hours=float(0.5)),
            datetime(2000, 1, 1, 0, 30),
        )
        self.assertEqual(
            datetime(2000, 1, 1, 0, 0, 0) + relativedelta(minutes=float(0.5)),
            datetime(2000, 1, 1, 0, 0, 30),
        )
        self.assertEqual(
            datetime(2000, 1, 1, 0, 0, 0, 0) + relativedelta(seconds=float(0.5)),
    ...
    def testSubtraction(self):
        self.assertEqual(
            ...
            relativedelta(
                years=-1, months=-2, days=7, hours=-4, minutes=-5, microseconds=-6
            ),
        )

    def testRightSubtractionFromDatetime(self):
        self.assertEqual(
            datetime(2000, 1, 2) - relativedelta(days=1), datetime(2000, 1, 1)
        )

    def testSubractionWithDatetime(self):
        self.assertRaises(
            TypeError, lambda x, y: x - y, (relativedelta(days=1), datetime(2000, 1, 1))
        )
    ...
    def testMultiplication(self):
        self.assertEqual(
            datetime(2000, 1, 1) + relativedelta(days=1) * 28, datetime(2000, 1, 29)
        )
        self.assertEqual(
    ...
    def testAbsoluteValueNegative(self):
        ...
        rd_expected = relativedelta(
            years=1, months=5, days=2, hours=3, minutes=5, seconds=2, microseconds=12
        )
    ...
    def testRelativeDeltaRepr(self):
        self.assertEqual(
            repr(relativedelta(years=1, months=-1, days=15)),
            "relativedelta(years=+1, months=-1, days=+15)",
        )

        self.assertEqual(
            repr(relativedelta(months=14, seconds=-25)),
            "relativedelta(years=+1, months=+2, seconds=-25)",
        )

        self.assertEqual(
            repr(relativedelta(month=3, hour=3, weekday=SU(3))),
    ...
    def testRelativeDeltaNormalizeFractionalDays(self):
        ...
        self.assertEqual(
            rd2.normalized(), relativedelta(days=1, hours=11, minutes=31, seconds=12)
        )
    ...
    def testRelativeDeltaNormalizeFractionalMinutes(self):
        ...
        self.assertEqual(
            rd2.normalized(), relativedelta(minutes=25, seconds=20, microseconds=25000)
        )
    ...
    def testRelativeDeltaNormalizeFractionalSeconds(self):
        ...
        self.assertEqual(
            rd1.normalized(), relativedelta(seconds=45, microseconds=25000)
        )
    ...
    def testRelativeDeltaFractionalPositiveOverflow2(self):
        ...
        self.assertEqual(
            rd3.normalized(),
            relativedelta(days=2, hours=2, minutes=0, seconds=2, microseconds=3),
        )
    ...
    def testRelativeDeltaFractionalNegativeOverflow(self):
        ...
        self.assertEqual(
            rd3.normalized(), relativedelta(days=-1, hours=-14, minutes=-45)
        )

        # Equivalent to (days=-1, hours=-14, minutes=+15)
        rd4 = relativedelta(days=-1.5, hours=-2.5, minutes=45)
        self.assertEqual(
            rd4.normalized(), relativedelta(days=-1, hours=-14, minutes=+15)
        )
    ...
    def testAddTimedeltaToUnpopulatedRelativedelta(self):
        ...
        expected = relativedelta(
            weeks=1, days=1, hours=1, minutes=1, seconds=1, microseconds=1001
        )
