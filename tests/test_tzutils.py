...
# Skip all tests if tzutils module doesn't exist yet
pytestmark = pytest.mark.skipif(
    True, reason="Skip these tests until tzutils module is properly installed"
)

try:
    from dateutil.tzutils import walltimedelta

    pytestmark = pytest.mark.skipif(False, reason="tzutils module available")
except ImportError:
    pass
...
def test_walltimedelta_different_timezones():
    ...
    with pytest.raises(
        ValueError, match="Datetimes are in different timezones"
    ):
        walltimedelta(start, end)
